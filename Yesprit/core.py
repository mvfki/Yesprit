...
#    namefile = open('4species_name.txt','r')
#    print(csv_file)
...
def searchAnno(species, keyword_search):
    ...
    conn.close()
    if species == 'P':
        dbPath_pombe = os.path.join(this_dir, 'data', species, 'ANNOTATION_noncoding.db')
        conn_pombe = connect(dbPath_pombe)
        c_pombe = conn_pombe.cursor()
        cmd_pombe = '''SELECT * 
        FROM ANNOTATION
        WHERE GenBank = "{0}" 
        OR ID = "{0}" 
        ...
        OR prod = "{0}"
        OR Symbol = "{0}";'''.format(keyword)
        results_pombe = c_pombe.execute(cmd_pombe).fetchall()
        conn_pombe.close()
        results_out = results_pombe
        print(results)
        print(results_pombe)
        if len (results) ==0 and len(results_pombe) !=0:
            results_out = results_pombe
        else:
            results_out = results   
    else:
...
def formatMultiResult(result):
    #df = DataFrame(result, columns=['ID', 'species', 'chromosome', 'start', 
    #                                   'end', 'strand', 'GenBank', 
    #                                   'Systematic ID', 'gene symbol'])
    #return len(df.index), str(df[['GenBank', 'Systematic ID', 'gene symbol']])
    
    ### Above were originally adopted methods, which is very easy to come up
    ### with. However, when packing the whole thing into Window executable, 
    ### the great "pandas" alone would take ~150Mb space, which is not 
    ### acceptable at all. So here it'll take some time for me to implement 
    ### some thing equivalent. 
    nResult = len(result)
    outString = ''
